...
var TradePrices = function (_widgetize$base) {
	...
	_createClass(TradePrices, [{
		...
		value: function attach(dom) {
			...
			this._xAxisLabel = this._xAxis.append('text').attr('y', 40).attr('class', 'axis-label').text('Trade Time');

			this._yAxis = this._graph.append('svg:g').attr('class', 'y axis').attr('transform', 'translate(' + MARGIN_LEFT + ',0)');

			this._yAxisLabel = this._graph.append('text').attr('transform', 'rotate(-90)').attr('y', 0).attr('dy', '1em').attr('class', 'axis-label').text('Trade Price');

			this._voronoiGroup = this._graph.append('svg:g').attr('class', 'voronoi').on('mouseover', function () {
		...
		value: function update() {
			...
			var voronoi = d3.geom.voronoi().x(function (d) {
				return _this3._xScale(d.time);
			}).y(function (d) {
				return _this3._yScale(d.price);
			...
			}).entries(data).sort(sortGroup);

			var line = lineFactory(this._xScale, this._yScale);

			var lines = this._graph.selectAll('path.line').data(dataGroup, function (exchange) {
				return exchange.key;
			});

			lines.enter().append('svg:path').attr('class', function (exchange) {
				return 'line line-' + exchange.key;
			}).attr('id', function (exchange) {
				return 'line_' + exchange.key;
			});

			lines.transition().attr('d', function (exchange) {
				return line(exchange.values);
			});
			...
			var legends = this._graph.selectAll('text.legend').data(dataGroup, function (exchange) {
				return exchange.key;
			});

			legends.enter().append('text').attr('y', 30).attr('class', function (exchange) {
				return 'legend legend-' + exchange.key;
			}).text(function (exchange) {
				return exchange.key;
			});

			legends.transition().attr('x', function (exchange, index) {
				return colSpace / 2 + index * colSpace;
			});

			legends.exit().remove();

			var voronoiPaths = this._voronoiGroup.selectAll('path').data(voronoi(this._data));

			voronoiPaths.enter().append('svg:path');

			voronoiPaths.attr('d', function (cord) {
...
function onMouseMove() {
	...
	var trade = d3.select(element).datum().point;

	this._toolTip.attr('transform', 'translate(' + this._xScale(xAxisValue(trade)) + ',' + this._yScale(yAxisValue(trade)) + ')');

	this._toolTopLabel.text(trade.price);
}
...
},{"lodash.tostring":67,"lodash.upperfirst":68}],67:[function(require,module,exports){
(function (global){
/**
 * lodash 4.1.1 (Custom Build) <https://lodash.com/>
 * Build: `lodash modularize exports="npm" -o ./`
...
},{"lodash.tostring":67}],69:[function(require,module,exports){
/**
...
},{"lodash.tostring":70}],70:[function(require,module,exports){
...
},{"lodash.tostring":72}],72:[function(require,module,exports){
arguments[4][67][0].apply(exports,arguments)
},{"dup":67}]},{},[6]);
